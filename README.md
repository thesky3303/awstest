...
| **Podman (Docker 호환 CLI: `docker`)** | **필수** — `scripts/img.sh`로 WAS(`ticketing-was`)·워커 이미지를 빌드·ECR 푸시할 때 (`podman-docker`가 `docker` 명령 제공) |

설치: [kubectl](https://kubernetes.io/docs/tasks/tools/) · [Helm](https://helm.sh/docs/intro/install/) · [Podman](https://podman.io/docs/installation)

**로컬 mysql (Red Hat 계열, 선택)**

**Podman (WAS·워커 이미지 — `img.sh`)**

`bash scripts/img.sh`는 `services/ticketing-was`, `services/worker-svc`에 대해 `docker build` / `docker push`를 실행합니다. **로컬에 Podman이 설치되어 있고**, `podman-docker`로 **`docker` 호환 명령이 PATH에 있어야** 합니다.
