...
if [[ -n "$ACCOUNT_ID" && -n "$REGION" ]]; then
  ...
  kubectl -n "$NS" set image deploy/write-api-burst-primary "write-api=${WAS_IMAGE}" >/dev/null || true
  kubectl -n "$NS" set image deploy/write-api-burst-secondary "write-api=${WAS_IMAGE}" >/dev/null || true
  kubectl -n "$NS" set image deploy/worker-svc "worker-svc=${WORKER_IMAGE}" >/dev/null || true
  kubectl -n "$NS" set image deploy/worker-svc-burst-primary "worker-svc=${WORKER_IMAGE}" >/dev/null || true
  kubectl -n "$NS" set image deploy/worker-svc-burst-secondary "worker-svc=${WORKER_IMAGE}" >/dev/null || true
fi
...
kubectl -n "$NS" rollout restart deploy/worker-svc deploy/worker-svc-burst-primary deploy/worker-svc-burst-secondary || true
kubectl -n "$NS" rollout restart deploy/read-api deploy/read-api-burst || true
kubectl -n "$NS" rollout restart deploy/write-api deploy/write-api-burst-primary deploy/write-api-burst-secondary || true
