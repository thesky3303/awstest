#!/usr/bin/env python3
"""
콘서트 "10초에 N천명 쓰기 시도" 부하 생성기 v6

v5(`sqs_load_real_concert5.py`)와의 차이(의도):
  - **동일한 실서버 경로**만 사용한다.
    - waiting-room enter/status
    - `/api/write/concerts/booking/commit` (QUEUED=SQS 접수)
  - **우회(commit6 등) 라우트는 사용하지 않는다.**
  - WR admit/폴링 기본값을 "대량 접수(로스 과대 계산 방지)" 쪽으로 맞춘다.
    - 필요하면 CLI로 v5처럼 짧게도 줄일 수 있다.

실행 예:
  WRITE_API_BASE_URL="http://write-api.ticketing.svc.cluster.local:5001" \\
  python3 sqs_load_real_concert6.py --show-id 8 -n 50 --duration-sec 10 \\
...
def _ensure_loadtest_users_fast(
    ...
    *,
    user_base: int,
    user_count: int,
    name_prefix: str = "sqs-load-concert6-",
) -> None:
...
def parse_args():
    p = argparse.ArgumentParser(
        description="콘서트 10초 버스트(HTTP/WAS) 부하 v6 (실경로 + WR 대량 admit 기본)",
    ...
    p.add_argument(
        ...
        default=float(
            os.getenv("BURST_HTTP_TIMEOUT_V6", os.getenv("BURST_HTTP_TIMEOUT", "45")) or 45
        ),
        help="개별 HTTP 타임아웃(초). v6 기본 45s — commit P99가 20s 근처일 때 20s면 타임아웃 로스가 뒤로 갈수록 커짐",
    )

    # ── Waiting Room ──
    p.add_argument(
        "--admit-rate", type=int,
        default=int(os.getenv("WR_ADMIT_RATE_V6", os.getenv("WR_ADMIT_RATE", "500000")) or 500000),
        help="waiting-room MANUAL admit_rate_per_sec (v6 기본은 대량 admit)",
    )
    ...
    # ── Status poll ──
    p.add_argument(
        "--status-poll-max-sec", type=float, default=30.0,
        help="permit 획득을 위한 status 폴링 최대 시간(초). v6는 기본을 넉넉히 잡아 admit 적체 로스를 줄인다.",
    )
    p.add_argument(
    ...
    p.add_argument(
        "--commit-retries",
        type=int,
        default=int(os.getenv("BURST_COMMIT_RETRIES_V6", os.getenv("BURST_COMMIT_RETRIES", "1")) or 1),
        help="commit 단계 재시도 횟수(0이면 재시도 없음). 과부하 구간에서 2+면 재시도가 오히려 전체 지연을 키울 수 있음",
    )
    p.add_argument(
        "--commit-retry-backoff-ms",
        type=int,
        default=int(os.getenv("BURST_COMMIT_RETRY_BACKOFF_MS_V6", os.getenv("BURST_COMMIT_RETRY_BACKOFF_MS", "60")) or 60),
        help="commit 재시도 기본 backoff(ms). 시도할수록 지수적으로 증가",
    )
...
async def main_async() -> int:
    ...
    async def _one(i: int, *, t0_fire: float, session: "aiohttp.ClientSession") -> None:
        ...
        for attempt in range(0, max(0, commit_retries) + 1):
            ...
            # WR 토큰 만료/유실로 429가 오면, 같은 uid로 재-enter → permit 재취득 후 재시도
            if code2 == 429 and api_code == "WAITING_ROOM_REQUIRED":
                ...
                if code0b == 200 and qref2:
                    ...
                    while poll_max > 0 and time.monotonic() < deadline2:
                        ...
                        if (
                            isinstance(st2, dict)
                            and st2.get("status") == "ADMITTED"
                            and st2.get("permit_token")
                        ):
            ...
            if attempt < commit_retries:
                # 지수 backoff + 소량 지터(동시 재시도 스파이크 완화)
    ...
    summary = {
        "script": "sqs_load_real_concert6.py",
        "show_id":        show_id,
