...
def parse_args():
    ...
    p.add_argument(
        ...
        help="permit 획득을 위한 status 폴링 최대 시간(초). 2초는 큐 적체 시 로스가 과대 계산될 수 있어 기본을 상향.",
    )
    p.add_argument(
    ...
    p.add_argument(
        "--commit-retries",
        type=int,
        default=int(os.getenv("BURST_COMMIT_RETRIES", "2") or 2),
        help="commit 단계 재시도 횟수(0이면 재시도 없음). timeout/500/503/WR_REQUIRED에 대해 backoff 재시도",
    )
    p.add_argument(
        "--commit-retry-backoff-ms",
        type=int,
        default=int(os.getenv("BURST_COMMIT_RETRY_BACKOFF_MS", "60") or 60),
    ...
    async def _one(i: int, *, t0_fire: float, session: "aiohttp.ClientSession") -> None:
        ...
        for attempt in range(0, max(0, commit_retries) + 1):
            ...
            if code2 == 429 and api_code == "WAITING_ROOM_REQUIRED":
                # 재-enter는 로스 카운터에 반영하지 않고 best-effort로만 수행
