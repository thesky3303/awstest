...
#   - write/read HPA(burst): minReplicas = -wr / -r,  maxReplicas = 그 min + 10  (10은 POD_MAX_DELTA)
#   - worker KEDA: minReplicaCount = -wk,  maxReplicaCount = -wk + 10,  paused 해제
#
# 버퍼 오버라이드: SET_TEST_CAP_POD_MAX_DELTA (기본 10), SET_TEST_CAP_NODE_MAX_DELTA (기본 5)

NS="${KUBECTL_NAMESPACE:-ticketing}"

POD_MAX_DELTA="${SET_TEST_CAP_POD_MAX_DELTA:-10}"
