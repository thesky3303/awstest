...
def _fetch_reserved_seat_keys_by_show(show_ids: List[int]) -> Dict[str, List[str]]:
    ...
    try:
        ...
        with conn.cursor() as cur:
            cur.execute(
                f"""
                SELECT show_id, seat_row_no, seat_col_no
                FROM concert_booking_seats
                WHERE show_id IN ({placeholders})
                  AND UPPER(COALESCE(status, '')) = 'ACTIVE'
                ORDER BY show_id, seat_row_no, seat_col_no
                """,
                tuple(show_ids),
            )
            rows = cur.fetchall() or []
...
def get_concert_detail(concert_id: int):
    ...
    show_dt = concert.get("venue_summary") or ""
    concert["release_date_display"] = show_dt
