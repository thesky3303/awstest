...
def _format_weekday(value):
    ...
    if not parsed:
        return ""
    return _WEEKDAY_MAP[parsed.weekday()]
...
def _fetch_theaters_from_db():
    ...
    try:
        ...
        for row in rows:
            ...
            if theater_id not in theaters_map:
                theaters_map[theater_id] = {
            ...
            if row.get("hall_id") is not None:
                theaters_map[theater_id]["halls"].append(
                    {
                        "hall_id": row["hall_id"],
...
def _fetch_theater_detail_from_db(theater_id):
    ...
    try:
        ...
        theater = {
            "theater_id": theater_rows[0]["theater_id"],
            "address": theater_rows[0]["address"],
            "halls": [],
        }

        for row in theater_rows:
            if row.get("hall_id") is None:
                continue

            theater["halls"].append(
                {
                    "hall_id": row["hall_id"],
                    "hall_name": row["hall_name"],
                    "total_seats": row["total_seats"],
        ...
        for row in schedule_rows:
            ...
            show_date_text = _format_date_text(row["show_date"])
            ...
            movie_entry = movies_map[movie_id]
            date_entry = next(
                (item for item in movie_entry["dates"] if item["show_date"] == show_date_text),
                None,
            )
            ...
            if date_entry is None:
                date_entry = {
                    ...
                    "month": _format_month(row["show_date"]),
                    "day": _format_day(row["show_date"]),
                    "week_day": _format_weekday(row["show_date"]),
                    "schedules": [],
                }
            ...
            date_entry["schedules"].append(
                {
                    ...
                    "start_time": _format_time(row["show_date"]),
                    "end_time": _make_end_time(row["show_date"], row["runtime_minutes"]),
                    "runtime_minutes": row["runtime_minutes"],
                    "total_count": row["total_count"] or row["total_seats"],
                }
            )
...
def refresh_theaters_cache():
    ...
    return {
        "cache_key": THEATERS_LIST_CACHE_KEY,
        "count": len(rows),
    }
...
def rebuild_theaters_cache():
    ...
    # 예매 화면 bootstrap(theaters:booking:bootstrap:v*) — 영화 목록 캐시가 먼저 채워진 뒤 실행되어야 함(rebuild-all 순서).
