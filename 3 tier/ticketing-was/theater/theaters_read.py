...
def _fetch_bootstrap_from_db(movie_map):
        ...
        with conn.cursor() as cur:
            ...
            cur.execute(
                ...
                SELECT
                    ...
                    h.created_at,
                    h.updated_at,
                    MAX(hs.seat_row_no) AS seat_rows,
                    MAX(hs.seat_col_no) AS seat_cols,
                    SUM(CASE WHEN UPPER(COALESCE(hs.status, 'ACTIVE')) = 'ACTIVE' THEN 1 ELSE 0 END) AS active_seat_count
                FROM halls h
                LEFT JOIN hall_seats hs
                    ON hs.hall_id = h.hall_id
                GROUP BY
                    h.hall_id,
                    h.theater_id,
                    h.hall_name,
    ...
    # 극장은 DB theaters 전 행을 내려줍니다(스케줄이 없는 극장도 목록에 표시).
...
def _build_theater_detail_payload(theater_id):
    ...
    hall_ids = {
        ...
        for item in bootstrap["halls"]
        if _to_int(item.get("theater_id")) == theater_id
    }

    halls = [
        item for item in bootstrap["halls"] if _to_int(item.get("hall_id")) in hall_ids
    ]
    schedules = [
        item for item in bootstrap["schedules"] if _to_int(item.get("hall_id")) in hall_ids
    ]
    movie_ids = {_to_int(item.get("movie_id")) for item in schedules}
    movies = [
        item for item in bootstrap["movies"] if _to_int(item.get("movie_id")) in movie_ids
    ]
    reserved_seats = {
        key: value
...
def get_theaters_remain_overrides():
    # Optional pipeline for future remain overrides.
    # Web expects this endpoint may not exist; returning an empty map prevents noisy 404 logs.
    return {}
...
if __name__ == "__main__":
    print(json.dumps(refresh_theaters_bootstrap_cache(), ensure_ascii=False))
