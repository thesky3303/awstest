        ...
        with conn.cursor() as cur:
            cur.execute(
                ...
                SELECT * FROM (
                    ...
                    WHERE b.user_id = %s
                      AND UPPER(COALESCE(b.book_status, '')) NOT IN %s

                    UNION ALL

                    SELECT
                        'concert' AS booking_kind,
                        cb.booking_id,
                        cb.created_at AS booking_date,
                        c.title AS movie_title,
                        cs.venue_address AS theater_address
                    FROM concert_booking cb
                    JOIN concert_shows cs ON cs.show_id = cb.show_id
                    JOIN concerts c ON c.concert_id = cs.concert_id
                    WHERE cb.user_id = %s
                      AND UPPER(COALESCE(cb.book_status, '')) NOT IN %s
                ) AS u
                ORDER BY u.booking_date DESC
                LIMIT 5
                """,
                (
                    user_id_int,
                    EXCLUDED_BOOKING_STATUSES_SQL,
                    user_id_int,
                    EXCLUDED_BOOKING_STATUSES_SQL,
                ),
            )
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                """
                SELECT
                    (SELECT COUNT(*) FROM booking WHERE user_id = %s)
                    + (SELECT COUNT(*) FROM concert_booking WHERE user_id = %s)
                    AS total_count
                """,
            ...
            movie_booking_ids = [
                ...
                if (row.get("booking_kind") or "movie") == "movie"
            ]
            concert_booking_ids = [
                row["booking_id"]
                for row in booking_rows
                if (row.get("booking_kind") or "movie") == "concert"
            ]

            seats_map: Dict[str, List[str]] = {}

            if movie_booking_ids:
                placeholders = ",".join(["%s"] * len(movie_booking_ids))
                cur.execute(
                    f"""
                    SELECT
                        bs.booking_id,
                    ...
                    ORDER BY bs.booking_id, hs.seat_row_no, hs.seat_col_no
                    """,
                    tuple(movie_booking_ids),
                )
                for sr in cur.fetchall():
                    bid = sr["booking_id"]
                    key = f"movie:{bid}"
                    if key not in seats_map:
                        seats_map[key] = []
                    seats_map[key].append(
                        f"{sr['seat_row_no']}-{sr['seat_col_no']}"
                    )

            if concert_booking_ids:
                placeholders = ",".join(["%s"] * len(concert_booking_ids))
                cur.execute(
                    f"""
                    SELECT booking_id, seat_row_no, seat_col_no
                    FROM concert_booking_seats
                    WHERE booking_id IN ({placeholders})
                    ORDER BY booking_id, seat_row_no, seat_col_no
                    """,
                    tuple(concert_booking_ids),
                )
                for sr in cur.fetchall():
                    bid = sr["booking_id"]
                    key = f"concert:{bid}"
            ...
            bookings.append(
                {
                    ...
                    "region_name": _derive_region_name(address),
                    "pay_yn": row.get("pay_yn"),
                    "paid_at": row.get("paid_at"),
                    "seats": seats_map.get(seat_key, []),
                }
            )
